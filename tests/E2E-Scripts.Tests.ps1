...
Describe "End-to-End Script Tests" {
    ...
    It "Convert-ScannedFIles.ps1 processes input files and creates JSON metadata" {
        ...
        Test-Path (Join-Path $script:resultPath "testovayapapka1") | Should -BeTrue
        Test-Path (Join-Path $script:resultPath "testovayapapka2") | Should -BeTrue

        # Check metadata directory exists
        Test-Path $script:metadataPath | Should -BeTrue

        # Check JSON metadata files were created
        Test-Path (Join-Path $script:metadataPath "testovayapapka1.json") | Should -BeTrue
        Test-Path (Join-Path $script:metadataPath "testovayapapka2.json") | Should -BeTrue
    ...
    It "ConvertTo-ZolaContent.ps1 converts JSON metadata to Zola markdown files" {
        ...
        Test-Path (Join-Path $script:zolaPath "testovayapapka1") | Should -BeTrue
        Test-Path (Join-Path $script:zolaPath "testovayapapka2") | Should -BeTrue

        # Check section index files
        Test-Path (Join-Path $script:zolaPath "testovayapapka1" "_index.md") | Should -BeTrue
        Test-Path (Join-Path $script:zolaPath "testovayapapka2" "_index.md") | Should -BeTrue

        # Check content pages were created
        $contentPages1 = @(Get-ChildItem -Path (Join-Path $script:zolaPath "testovayapapka1") -File -Filter "*.md" | Where-Object { $_.Name -ne "_index.md" })
